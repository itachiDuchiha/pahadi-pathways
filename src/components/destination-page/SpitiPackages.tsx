...
export default function SpitiPackages() {
  const spitiPackages = packages.filter((pkg) =>
    pkg.itinerary.some((stop) => {
      const location = stop.toLowerCase();

      return (
        location.includes("spiti") ||
        location.includes("kaza") ||
        location.includes("tabo") ||
        location.includes("dhankar") ||
        location.includes("langza") ||
        location.includes("hikkim") ||
        location.includes("komik")
      );
    })
  );
        ...
        {spitiPackages.length > 0 ? (
          ...
          <div className="mt-6 rounded-3xl border border-[#10264A]/10 bg-white p-10 text-center">
            ...
            <p className="text-gray-600">
              Spiti packages will be available soon.
            </p>
